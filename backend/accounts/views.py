...
def _send_friend_request_notification(sender, receiver, notification_type, title, message, extra_data=None):
    ...
    try:
        ...
        logger.exception("Failed to create notification for user=%s", receiver.id)
    ...
    try:
        ...
        logger.exception("Failed to broadcast push notification for user=%s", receiver.id)
...
class CloudinaryProxyView(APIView):
    permission_classes = []
...
class GuardianDashboardView(APIView):
    ...
    def get(self, request):
        logger.info("GuardianDashboardView GET: user=%s", request.user.username)
        try:
            user = request.user
            ...
            logger.exception("GuardianDashboardView GET: error")
            return Response(
                {"error": str(e)},
...
class GuardianProfileView(APIView):
    ...
    def put(self, request):
        logger.info("GuardianProfileView PUT: user=%s", request.user.username)
...
class FamilyMemberListCreateView(APIView):
    ...
    def get(self, request):
        logger.info("FamilyMemberListCreateView GET: user=%s", request.user.username)
        try:
            profile = getattr(request.user, "guardian_profile", None)
            ...
            logger.exception("FamilyMemberListCreateView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def post(self, request):
        logger.info("FamilyMemberListCreateView POST: user=%s", request.user.username)
        try:
            profile = getattr(request.user, "guardian_profile", None)
            ...
            logger.exception("FamilyMemberListCreateView POST: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class SafePlaceListCreateView(APIView):
    ...
    def get(self, request):
        logger.info("SafePlaceListCreateView GET: user=%s", request.user.username)
        try:
            profile = getattr(request.user, "guardian_profile", None)
            ...
            logger.exception("SafePlaceListCreateView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def post(self, request):
        logger.info("SafePlaceListCreateView POST: user=%s", request.user.username)
        try:
            profile = getattr(request.user, "guardian_profile", None)
            ...
            logger.exception("SafePlaceListCreateView POST: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class EmergencyContactListCreateView(APIView):
    ...
    def get(self, request):
        logger.info("EmergencyContactListCreateView GET: user=%s", request.user.username)
        try:
            profile = getattr(request.user, "guardian_profile", None)
            ...
            logger.exception("EmergencyContactListCreateView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def post(self, request):
        logger.info("EmergencyContactListCreateView POST: user=%s", request.user.username)
        try:
            profile = getattr(request.user, "guardian_profile", None)
            ...
            logger.exception("EmergencyContactListCreateView POST: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class FamilyInvitationView(APIView):
    ...
    def post(self, request):
        logger.info("FamilyInvitationView POST: request.data=%s", request.data)
        ...
        logger.info("FamilyInvitationView POST: resolved child=%s", child)
        ...
        try:
            ...
            logger.info("FamilyInvitationView POST: created invitation id=%s", invitation.id)
        except IntegrityError:
            logger.warning("FamilyInvitationView POST: duplicate invitation for guardian=%s child=%s", request.user.id, child.id)
            return Response(
                {"error": "Invitation already exists."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except Exception as e:
            logger.exception("FamilyInvitationView POST: failed to create invitation")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        try:
            ...
            logger.info("FamilyInvitationView POST: created notification for child=%s", child.id)
        except Exception as e:
            logger.exception("FamilyInvitationView POST: failed to create notification")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        try:
            ...
            logger.info("FamilyInvitationView POST: broadcast push notifications sent")
        except Exception as e:
            logger.exception("FamilyInvitationView POST: failed to broadcast push notifications")

        try:
            response_data = FamilyInvitationSerializer(invitation).data
            logger.info("FamilyInvitationView POST: returning response status=201")
            return Response(
                response_data,
                status=status.HTTP_201_CREATED,
            )
        except Exception as e:
            logger.exception("FamilyInvitationView POST: failed to serialize invitation")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class FamilyMembersView(APIView):
    ...
    def get(self, request):
        logger.info("FamilyMembersView GET: user=%s", request.user.username)
        try:
            user = request.user
            if user.is_guardian:
            ...
            logger.exception("FamilyMembersView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class RemoveFamilyMemberView(APIView):
    ...
    def delete(self, request, user_id):
        logger.info("RemoveFamilyMemberView DELETE: user=%s target=%s", request.user.username, user_id)
        try:
        ...
        try:
            ...
            logger.exception("RemoveFamilyMemberView DELETE: failed to create activity log")
...
class LocationPermissionView(APIView):
    ...
    def get(self, request):
        logger.info("LocationPermissionView GET: user=%s", request.user.username)
        try:
            user = request.user
            if user.is_guardian:
            ...
            logger.info("LocationPermissionView GET: returning %d permissions", permissions.count())
            return Response(serializer.data)
        except Exception as e:
            logger.exception("LocationPermissionView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def patch(self, request):
        logger.info("LocationPermissionView PATCH: user=%s, data=%s", request.user.username, request.data)
        user = request.user
        if user.is_guardian:
            return Response(
        ...
        try:
            ...
            logger.info("LocationPermissionView PATCH: permission updated for user=%s guardian=%s", user.username, guardian.username)
            return Response(LocationPermissionSerializer(permission).data)
        except Exception as e:
            logger.exception("LocationPermissionView PATCH: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class SOSAlertListCreateView(APIView):
    ...
    def get(self, request):
        logger.info("SOSAlertListCreateView GET: user=%s", request.user.username)
        try:
            if request.user.is_guardian:
            ...
            logger.exception("SOSAlertListCreateView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def post(self, request):
        logger.info("SOSAlertListCreateView POST: user=%s", request.user.username)
        if request.user.is_guardian:
            return Response(
        ...
        try:
            ...
            logger.info("SOSAlertListCreateView POST: created alert id=%s", alert.id)
        except Exception as e:
            logger.exception("SOSAlertListCreateView POST: failed to create alert")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        for guardian in guardians:
            try:
                ...
                logger.exception("SOSAlertListCreateView POST: failed to create notification for guardian=%s", guardian.username)
            ...
            try:
                ...
                logger.exception("SOSAlertListCreateView POST: failed to broadcast push for guardian=%s", guardian.username)
...
class ResolveSOSAlertView(APIView):
    ...
    def post(self, request, pk):
        logger.info("ResolveSOSAlertView POST: user=%s pk=%s", request.user.username, pk)
        try:
        ...
        try:
            ...
            logger.info("ResolveSOSAlertView POST: resolved alert id=%s", alert.id)
            return Response({"message": "SOS alert resolved."})
        except Exception as e:
            logger.exception("ResolveSOSAlertView POST: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class RouteHistoryView(APIView):
    ...
    def get(self, request):
        logger.info("RouteHistoryView GET: user=%s", request.user.username)
        ...
        try:
            ...
            logger.exception("RouteHistoryView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class ActivityLogView(APIView):
    ...
    def get(self, request):
        logger.info("ActivityLogView GET: user=%s", request.user.username)
        try:
            user = request.user
            if user.is_guardian:
            ...
            logger.exception("ActivityLogView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class PermissionRequestView(APIView):
    ...
    def get(self, request):
        logger.info("PermissionRequestView GET: user=%s", request.user.username)
        try:
            user = request.user
            if user.is_guardian:
            ...
            logger.exception("PermissionRequestView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def post(self, request):
        logger.info("PermissionRequestView POST: user=%s", request.user.username)
        user = request.user
        if user.is_guardian:
            ...
            try:
                ...
                logger.exception("PermissionRequestView POST: failed to create request")
                return Response(
                    {"error": str(e)},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
            ...
            try:
                ...
                logger.exception("PermissionRequestView POST: failed to create notification")
            ...
            if action == "accept":
                try:
                    ...
                    logger.exception("PermissionRequestView POST: failed to process accept action")
                    return Response(
                        {"error": str(e)},
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    )
...
class FamilyMapDataView(APIView):
    ...
    def get(self, request):
        logger.info("FamilyMapDataView GET: user=%s", request.user.username)
        ...
        try:
            ...
            logger.info("FamilyMapDataView GET: returning %d family members", len(data))
            return Response(data)
        except Exception as e:
            logger.exception("FamilyMapDataView GET: error")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
